...
const db1 ="mongodb+srv://vivek:vivek@cluster1-zfbnm.mongodb.net/movies?retryWrites=true" 
mongoose.connect(db, err=>{
    if(err){ console.error(err)}
    else{ console.log('Mongodb Connected to user db')}
})
mongoose.connect(db1, err=>{
    if(err){ console.error(err)}
    else{ console.log('Mongodb Connected to movies db')}
})
router.get('/',(req,res)=>{
    res.send('From Api')
})
